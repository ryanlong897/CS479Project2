...
int ImageParser::ReadImage(std::string fname, Image& image)
{
    ...
    charImage = (unsigned char *) new unsigned char [M*N*3];

    ifp.read( reinterpret_cast<char *>(charImage), (M*N)*sizeof(unsigned char)*3);
    ...
    for (i=0; i<N; i++)
    {
        for (j=0; j<M*3; j+=3) 
        {
            for (int c = 0; c < 3; c++)
            {
                val = (int)charImage[i*M+j+c];
                image.setPixelVal(c, i, j/3, val);     
            }
        }
    }
...
int ImageParser::WriteImage(std::string fname, Image& image)
{
    ...
    for(i=0; i<N; i++)
    {
        for(j=0; j<M*3; j+=3) {
            for (int c = 0; c < 3; c++)
            {
                image.getPixelVal(c, i, j/3, val);
                charImage[i*M+j+c]=(unsigned char)val;
            }
        }
    }
